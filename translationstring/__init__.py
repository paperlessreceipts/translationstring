...
class TranslationString(text_type):
    ...
    This class can be subclassed to, for example, provide new interpolation
    schemes.
    """
    ...
    def __mod__(self, options):
        ...
        return type(self)(self, mapping=mapping)
...
def TranslationStringFactory(factory_domain, cls=TranslationString):
    """ Create a factory which will generate translation strings
    without requiring that each call to the factory be passed a
    ``domain`` value. This constructor accepts a single required argument:
    ``domain`` which will be used as the ``domain`` values of translation
    string objects generated by the ``__call__`` of this class. The type of the
    translation string objects created by the factory can be changed from the
    default :class:`translationstring.TranslationString`, by passing a subclass
    of :class:`translationstring.TranslationString` to `cls`. The ``msgid``,
    ``mapping``, and ``default`` values provided to the ``__call__`` method of
    an instance of this class have the meaning as described by the constructor
    of the :class:`translationstring.TranslationString`"""
    def create(msgid, mapping=None, default=None, context=None):
        ...
        return cls(msgid, domain=domain, default=default,
                   mapping=mapping, context=context)
    return create

def ChameleonTranslate(translator, cls=TranslationString):
    """
    ...
    untranslated). The optional ``cls`` argument is used to create a
    translation string when the ``msgid`` argument to the returned ``translate``
    function is a unicode object, and defaults to:
    ...
    def translate(msgid, domain=None, mapping=None, context=None,
        ...
        if not hasattr(tstring, 'interpolate'):
            tstring = cls(msgid, domain, default, mapping, context)
...
def Translator(translations=None, policy=None, cls=TranslationString):
    """
    ...
    translation string. The optional ``cls`` argument is used to
    create a translation string when the ``tstring`` argument to the
    returned ``translator`` function is simple string, and defaults to:
    :class:`translationstring.TranslationString`.
    """
    if policy is None:
    ...
    def translator(tstring, domain=None, mapping=None, context=None):
        if not hasattr(tstring, 'interpolate'):
            tstring = cls(tstring, domain=domain, mapping=mapping, context=context)
        elif mapping:
            if tstring.mapping:
                ...
                new_mapping = mapping
            tstring = type(tstring)(tstring, domain=domain, mapping=new_mapping, context=context)
        translated = tstring
...
def Pluralizer(translations=None, policy=None, cls=TranslationString):
    """
    ...
    ``cls`, if supplied, should be a subclass of
    :class:`translationstring.TranslationString`. If the returned ``pluralizer``
    is passed simple strings for ``singular`` or ``plural``, it is used to
    turn these into translation strings.
    ...
    def pluralizer(singular, plural, n, domain=None, mapping=None, context=None):
        ...
        translated = policy(translations, singular, plural, n, domain, context)
        if translated is singular:
            type_ = type(singular)
        else:
            type_ = type(plural)
